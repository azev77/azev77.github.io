...
(when the bias is big enough in the opposite direction of the estimated effect): 
...
<br>
    $$
    \begin{align*}
    \hat{\beta}_{xy}                         &=  1\%                    & \text{Estimated effect excluding Z}
    \\
    \beta_{xy}                               &=  -5\%                   & \text{Causal effect, estimated when including Z}
    \\
    \text{Bias}                              &= \delta_{xz} \beta_{zy}  &= 60\% \times 10\%
    \\
