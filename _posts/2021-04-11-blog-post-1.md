...
| pdf(Truncated(Normal(μ,σ),0,1),x) 	| google for a package<br>     ...<br>     dtrunc(x, spec="norm", a=0, b=1, mean=μ, sd=σ) 	|
| MixtureModel([Normal(μ1,σ1),   Normal(μ2,σ2)],[1/2,1/2]) 	| google for a package<br>     … 	|
| pdf(BetaBinomial(n,α,β),x) 	| google for a package<br>     … 	|
